...
static char *          /* Return ls(1)-style string for file permissions mask */
...
displayStatInfo(const struct stat *sb)
{
    ...
    char* ct = (char*) malloc(50);
    ct = ctime(&sb->st_atime);
    ct[strlen(ct)-1]='\0';
    printf("Last file access:         %s %ldns\n", ct, sb->st_atim.tv_nsec);
    ct = ctime(&sb->st_mtime);
    ct[strlen(ct)-1]='\0';
    printf("Last file modification:   %s %ldns\n", ct, sb->st_mtim.tv_nsec);
    ct = ctime(&sb->st_ctime);
    ct[strlen(ct)-1]='\0';
    printf("Last status change:       %s %ldns\n", ct, sb->st_ctim.tv_nsec);
}
